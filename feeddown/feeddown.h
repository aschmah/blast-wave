...
// mass, spinType (2 J + 1), and name for each particle id (pdg code) in Pythia 8
// with these data the feeddown class does not depend on the Pythia 8 library 
#include "pythia_particle_data.h"
...
class feeddown {
    ...
    Double_t bw_pars[20]; // blast wave parameters (0 = mass per defintion)
    Double_t sm_pars[2];  // chemical freeze-out temperature (0) and potential (1)
    Int_t n_bw_pars; // number of blast-wave parameters including mass ans spinType

    // models need to have a certain format (see below)
    Double_t (*dndpt_model)(Double_t *x, Double_t *par);
    Double_t (*dndpt_norm)(Double_t *par);
    ...
    void set_dndpt_and_v2_primary_from_model(Double_t, Double_t, TH1D *, TH1D *);
  ...
  public:
    feeddown() : current_model{undefined}, maximum_mother_mass{10000.} {};
    ...
    void set_dndpt_norm(Double_t model(Double_t *par));
    void set_dndpt_and_v2_model(void model(Double_t *x, Double_t *par, Double_t &dndpt, Double_t &v2));
    void set_bw_model_parameters(Double_t p[], const Int_t npar);
    void set_sm_model_parameters(Double_t Tch, Double_t mu = 0) {
        sm_pars[0] = Tch;
        sm_pars[1] = mu;
    };
