...
void feeddown::set_bw_model_parameters(Double_t p[], const Int_t npar) {
    if (npar <= 20) {
        // copy model parameters, index 0 reserved for mass (set later)
        for (Int_t i = 2; i <= npar+1; ++i)
            bw_pars[i] = p[i - 2];
    } else {
        std::cout << "ERROR: too many model parameters" << std::endl;
    }
    n_bw_pars = npar + 2; // including mass and spinType
}
...
// set model for calculating the yield (dN/dpT)
void feeddown::set_dndpt_norm(Double_t norm(Double_t *par)) { dndpt_norm = norm; }
...
void feeddown::set_dndpt_and_v2_primary_from_model(Double_t m, Double_t spinType, TH1D *h_dndpt, TH1D *h_v2) {

    // set mass in the array of the blast wave parameters
    bw_pars[0] = m;
    bw_pars[1] = spinType;

    Double_t Tch = sm_pars[0];
    Double_t mu = sm_pars[1];

    // blast-wave + statistical model parameters
    Double_t bw_sm_pars[20];
    for (Int_t i=0; i<n_bw_pars; ++i) bw_sm_pars[i] = bw_pars[i];
    bw_sm_pars[n_bw_pars] = Tch;
	bw_sm_pars[n_bw_pars + 1] = mu;

    // first get normalization of dN/dpT
    Double_t norm = 1;
    if (dndpt_norm)
        norm = (*dndpt_norm)(bw_sm_pars);
    else
        std::cout << "dndpt_norm not defined ... " << std::endl;

    if (current_model == dndpt) {

        for (Int_t i = 1; i <= h_dndpt->GetXaxis()->GetNbins(); ++i) {
            Double_t pt = h_dndpt->GetBinCenter(i);
            Double_t yield = (*dndpt_model)(&pt, bw_pars);
            h_dndpt->SetBinContent(i, norm * yield );
        ...
        for (Int_t i = 1; i <= h_dndpt->GetXaxis()->GetNbins(); ++i) {
            Double_t pt = h_dndpt->GetBinCenter(i);
            (*dndpt_and_v2_model)(&pt, bw_pars, dndpt, v2);
            h_dndpt->SetBinContent(i, norm * dndpt);
...
void feeddown::calc_feeddown_hist(const Int_t id) {
    ...
    Double_t m = pythia_particle[id].mass;
    Double_t spinType = pythia_particle[id].spinType;
    set_dndpt_and_v2_primary_from_model(m, spinType, &h_dndpt_primary, &h_v2_primary);
...
void feeddown::calc_feeddown(const Int_t id, const Double_t &pt, Double_t &dndpt_primary, Double_t &dndpt_secondary,
    ...
    // mass and spin type of the considered primary particle
    Double_t m = pythia_particle[id].mass;
    Double_t spinType = pythia_particle[id].spinType; // spinType = 2 J + 1
    bw_pars[0] = m; // set mass in the array of the blast wave parameters
	bw_pars[1] = spinType;
    ...
    //
    // calculate primary particle yield
    //
    Double_t pttmp = pt;
    ...
    if (current_model == dndpt) {
        dndpt_primary = norm * (*dndpt_model)(&pttmp, bw_pars);
    } else if (current_model == dndpt_and_v2) {
        (*dndpt_and_v2_model)(&pttmp, bw_pars, dndpt_primary, v2_primary);
        dndpt_primary *= norm;
    } else {
        std::cout << "ERROR: model not defined" << std::endl;
    }
...
// We calculate the feeddown either over the full pt range (as histogram) or for a single pt bin.
// In both cases the following function is used
void feeddown::calc_feeddown_in_pt_sec_index_range(const Int_t id, const Int_t j_min, Int_t j_max,
    ...
    // get mass of the particle for which we calculate the feeddown contribution
    Double_t m_particle = pythia_particle[id].mass;
    ...
    for (auto const &mothers : decay_hist_dndpt_secondary) {
        ...
        Double_t m = pythia_particle[id_mother].mass;
        Double_t spinType = pythia_particle[id_mother].spinType; // spinType = 2 J + 1

        // ther can only be feeddown from particles with masses above the mass of the considered particle
        if (m < m_particle)
            continue;

        // reject particle if its mass is above the set mass limts
        if (m > maximum_mother_mass)
            continue;

        // if set of mother particle is not empty check whether mother particle is in the set
        if (mother_particles.size() != 0 && mother_particles.count(id_mother) == 0)
            continue;

        // calculate dndpt and v2 for this mother particle
        h_dndpt_mother->Reset();
        h_v2_mother->Reset();
        set_dndpt_and_v2_primary_from_model(m, spinType, h_dndpt_mother, h_v2_mother);
